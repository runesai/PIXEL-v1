...
def detect_department(text):
    """
    Detects the department in the given text using all department keywords/abbreviations/synonyms.
    Returns the canonical department name if found, else None.
    """
    dept_keywords_map = get_department_keywords_map()
    text_l = text.lower()
    # Direct substring match
    for keyword, canonical in dept_keywords_map.items():
...
def get_combined_faqs(sender_id):
    ...
    for tab in ['Department', 'Laboratory']:
        dept_faqs += get_faqs_for_user(sender_id, tab)  # You may need to adjust this
...
_FAQ_CACHE = None
_FAQ_CACHE_TIME = 0
_FAQ_CACHE_TTL = 600  # seconds (reduced for faster Google Sheets update reflection during testing)

_NORMALIZATION_CACHE = None
_NORMALIZATION_CACHE_TIME = 0
_NORMALIZATION_CACHE_TTL = 600  # seconds (reduced for faster Google Sheets update reflection during testing)
                ...
                if messaging_event.get("message"):
                    ...
                    if questions:
                        ...
                        for q in questions:
                            ...
                            # Detect department for each question
                            ...
                            if dept_key:
                                spreadsheet_id = dept_map[dept_key]
                                print(f"[DEBUG][webhook] Using department sheet: '{dept_key}' (ID: {spreadsheet_id}) for question: {q}")
                                # Fetch department-specific FAQs
                                faqs_dept = get_dept_faqs_cached(spreadsheet_id)
                                faqs_to_use = faqs_dept + faqs  # Prefer dept, fallback to combined
                            else:
                                print(f"[DEBUG][webhook] No department detected for question: {q}")
                                faqs_to_use = faqs
                            q_norm = normalize_query(q, normalization_dict)
                            print(f"[DEBUG][webhook] Normalized question: {repr(q_norm)}")
                            print(f"[DEBUG][webhook] FAQ cache size: {len(faqs_to_use)}")
                            faq_match = match_faq(q_norm, faqs_to_use, lang, normalization_dict=normalization_dict)
                            if faq_match:
                                ...
                                non_faq_questions.append((q, dept_key))
                        ...
                        for q, dept_key in non_faq_questions:
                            ...
                            if is_cpe_related(q, cpe_keywords):
                                # Log unanswered to the correct department sheet
                                log_unanswered_for_user(sender_id, q, lang)
                                system_prompt = PIXEL_PERSONALITY[lang]
                                ...
                                try:
                                    ...
                                    answers.append("Sorry, I encountered an error. Please try again.")
                            else:
...
# --- Aggressive caching for department FAQ fetches ---

_DEPT_FAQ_CACHE = {}
_DEPT_FAQ_CACHE_TIME = {}
_DEPT_FAQ_CACHE_TTL = 600  # 10 minutes

def get_dept_faqs_cached(spreadsheet_id):
    """
    Returns a list of FAQ dicts for a department spreadsheet_id, using cache.
    """
    import time
    now = time.time()
    if (
        spreadsheet_id not in _DEPT_FAQ_CACHE or
        now - _DEPT_FAQ_CACHE_TIME.get(spreadsheet_id, 0) > _DEPT_FAQ_CACHE_TTL
    ):
        service = get_sheets_service()
        sheet = service.spreadsheets()
        try:
            result = sheet.values().get(
                spreadsheetId=spreadsheet_id, range='FAQ!A2:C'
            ).execute()
            values = result.get('values', [])
            faqs_dept = []
            for row in values:
            ...
            print(f"[ERROR][webhook] Could not fetch department FAQ: {e}")
            faqs_dept = []
        _DEPT_FAQ_CACHE[spreadsheet_id] = faqs_dept
        _DEPT_FAQ_CACHE_TIME[spreadsheet_id] = now
    return _DEPT_FAQ_CACHE[spreadsheet_id]
