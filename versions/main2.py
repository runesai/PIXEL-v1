...
def translate_text(text, source_lang):
    """Translate text between English and Filipino"""
    try:
        tokenizer = global_models['tokenizer']
        model = global_models['model']
        ...
        inputs = tokenizer(f"{src_lang} {text}", return_tensors="pt", padding=True)
...
def detect_language_enhanced(text):
    """Enhanced language detection with Filipino patterns from sheets"""
    try:
        ...
        # Check against Filipino patterns
        fil_variations = language_variations.get('fil', {})
        for word in words:
            # Check if word is in Filipino variations
            if word in fil_variations:
                scores['fil'] += 2  # Higher weight for exact matches
            
            # Check word against categories
            for category, terms in language_variations['categories'].items():
                if word in terms:
                    if category in ['particle', 'pronoun']:  # Filipino-specific categories
                        scores['fil'] += 1.5
                    elif category in ['politeness']:  # Strong Filipino indicators
                        scores['fil'] += 2
        
        # Try standard language detection as backup
        try:
            ...
            pass  # Ignore if standard detection fails
