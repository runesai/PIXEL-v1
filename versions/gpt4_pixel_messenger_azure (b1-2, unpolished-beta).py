...
# --- Helper: Detect if a question is just an intent-to-ask phrase ---
                        ...
                        for idx, (q, a) in enumerate(zip(answer_questions, answer_texts)):
                            ...
                            if is_acad_dept_lab:
                                ...
                                try:
                                    ...
                                    fallback_prompt = (
                                        "Sorry, I couldn't understand your message. If you have any CpE or tech questions, I'm here to help!"
                                    )
...
def format_list_answers(questions, answers, instruction=None):
    ...
    for idx, (q, a) in enumerate(zip(questions, answers), 1):
        ...
        # For all but the first answer, add a transition phrase and never a greeting
        if idx > 1:
            transition = get_transition_phrase(idx, q)
            a = strip_greeting(a.strip())
            a = f"{transition} {a}"
...
# --- Helper: Get all academic/department/lab keywords from Language Sheet ---
